...
(function() {
  ...
  angular.module('contactModule').controller('ContactListCtrl', [
    ...
    '$scope', '$http', '$q', '$routeParams', 'contactManager', '$location', '$log', 'ModelBase', 'Restangular', '$upload', '$modal', '$rootScope', function($scope, $http, $q, $routeParams, contactManager, $location, $log, ModelBase, Restangular, $upload, $modal, $rootScope) {
      ...
      $scope.save = function() {
        var contactData, fields, _i, _len, _ref, _ref1;
        fields = ['surname', 'givenname', 'company', 'department', 'title', 'phone', 'mobile', 'fax', 'origin', 'email', 'address', 'birthday', 'region', 'website', 'qq', 'weibo', 'im'];
        $scope.progressBar.start();
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          contactData = _ref[_i];
          if (_ref1 = contactData.key, __indexOf.call(fields, _ref1) >= 0) {
        ...
        if ($scope.contact.data == null) {
          ...
          return Restangular.one('contacts', $scope.contact.id).all('contactdata').post($scope.contact.dataObj).then(function(contactdata) {
            return $scope.contact.put().then($scope.reload);
          });
        } else {
          return $scope.contact.dataObj.put().then(function(contactData) {
            return $scope.contact.put().then($scope.reload);
          });
        }
      };
